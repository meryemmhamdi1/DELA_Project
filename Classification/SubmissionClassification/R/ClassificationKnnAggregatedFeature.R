...
knnFit=train(x=db.train[,train_fs],
            y=db.train$Class,
            method = "knn",
            metric="ROC",
            trControl = ctrl,
...
#knnFit <- train(x=db.train[,fs],y=db.train$improved, method = "rf")#, trControl = ctrl, preProcess = c("center","scale"), tuneLength = 20)


preds_train_knn= predict(knnFit, newdata = db.train[,train_fs]);
table(preds_train_knn)

auc(roc(preds_train_knn, db.train$Class));
