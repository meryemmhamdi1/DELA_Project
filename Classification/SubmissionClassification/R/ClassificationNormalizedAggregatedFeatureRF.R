...
fs = c('Improved2','SeenVideo','NVideoAndForum_','IsLastSubm','NumberOfThreadsLaunched','SubmissionNumber','TotalTimeVideo','EngagementIndex')

#db$Improved2 =  ifelse(db$Improved==0 ,0, 1 )

up_train <- upSample(x=db[train_fs],
                     y= db$Improved)

#library(corrplot)
#corrplot(cor(up_train[fs]),order="hclust")
...
model_rf = train(x= db.train[,train_fs],
                 y= db.train$Class,
                 method = customRF,
                 metric="ROC",
                 trControl = ctrl_rf,
...
print(model_rf);   plot(model_rf,xlab="Number of Variables tried (mtry)",ylab="AUC (Cross-Validation)")  
...
ggplot(lda_data, aes(x = Training_Size, y = ROC, color = Data)) + 
  ...
  xlab("Number of submission records")+
  ylab("AUC Score")+
  theme_bw()
...
write.csv(kaggleSubmission,file='/media/diskD/EPFL/Fall 2016/DELA/DELA_Project/Classification/Results/classifier_results_final.csv', row.names = F)
